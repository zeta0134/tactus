...
def read_bigfloor(map_filename):
    ...
    return BigFloor(name=safe_label, width=map_width, height=map_height, tiles=combined_tiles)
...
def write_bigfloor(tilemap, output_file):
    output_file.write(ca65_label("bigfloor_"+tilemap.name) + "\n")
...
if __name__ == '__main__':
    # DEBUG TEST THINGS
    if len(sys.argv) != 3:
      print("Usage: bigfloor.py input.tmx output.txt")
      sys.exit(-1)
    input_filename = sys.argv[1]
    output_filename = sys.argv[2]

    tilemap = read_bigfloor(input_filename)

    with open(output_filename, "w") as output_file:
      write_bigfloor(tilemap, output_file)