...
def fancy_sin_log(log_multiplier, initial_amplitude, final_amplitude, step_count, offset=0):
    return [round(math.sin(math.log10(i+1) * log_multiplier + offset) * ((final_amplitude * i / step_count) + (initial_amplitude * (step_count - i) / step_count))) for i in range(0, step_count)]
...
def distorted_bank_pairs(log_multiplier, initial_amplitude, final_amplitude, step_count, offset=0):
    bank_offsets = fancy_sin_log(log_multiplier, initial_amplitude, final_amplitude, step_count, offset)
    #bank_offsets.reverse()
    clamped_offsets = [i % 16 for i in bank_offsets]
    dpairs = difference_pairs(bank_offsets, next_bank)
    return dpairs
...
def print_common_bank_tables(effect_name, output_file):
    print(ca65_label(f"{effect_name}_ppumask_common"), file=output_file)
    print("  .repeat 64", file=output_file)
    print("  .byte $1E", file=output_file)
    print("  .endrepeat", file=output_file)
    ...
    print(ca65_label(f"{effect_name}_irq_common"), file=output_file)
    print("  .byte >full_scroll_and_ppumask_irq", file=output_file)
    print("  .repeat 63", file=output_file)
    print("  .byte >title_chr0_lo_bank_only", file=output_file)
    print("  .endrepeat", file=output_file)
    print("", file=output_file)
    # also unused
    print(ca65_label(f"{effect_name}_scrolly_common"), file=output_file)
    print("  .repeat 64", file=output_file)
    print("  .byte 0", file=output_file)
    print("  .endrepeat", file=output_file)
    print("", file=output_file)
...
def print_bank_distortion_table(effect_name, frame_number, dpairs, output_file):
    scrollx_values = [0] + [p[0] % 16 for p in dpairs]
    scanline_numbers = [4] + [p[1] + 32 for p in dpairs]
    print(ca65_label(f"{effect_name}_scrollx_frame_{frame_number}"), file=output_file)
    pretty_print_table(scrollx_values, output_file, width=32)
    print(ca65_label(f"{effect_name}_scanline_frame_{frame_number}"), file=output_file)
    pretty_print_table(scanline_numbers, output_file, width=32)

def print_bank_distortion_frames(effect_name, num_frames, output_file):
    for i in range(0, num_frames):
        labels = [
            f"{effect_name}_scrollx_frame_{i}",
            f"{effect_name}_scrolly_common",
            f"{effect_name}_scanline_frame_{i}",
            f"{effect_name}_ppumask_common",
            f"{effect_name}_irq_common"]
        print(f"{effect_name}_frame_{i}:", file=output_file)
...
effect_name = "flaming_text"
period = 20     # height of the sine wave
effect_duration = 60 # playback frames/duration (should usually match height for looping)
start_amplitude = 1 
...
with open(f"../prg/raster/{effect_name}.incs", "w") as output_file:
    ...
    # for the fancy schmancy title bank switching distortion
    print_bank_distortion_frames(effect_name, effect_duration, output_file)
    print_common_bank_tables(effect_name, output_file)
    ...
    for i in range(0, effect_duration):
        ...
        start_amplitude = 10
        end_amplitude = 6
        offset = (i / effect_duration) * (math.pi * 2.0)
        distorted_banks = distorted_bank_pairs(log_multiplier, start_amplitude, end_amplitude, 48, offset)
        print_bank_distortion_table(effect_name, i, distorted_banks, output_file)
        framesets.append(distorted_banks)
