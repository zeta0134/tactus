...
$(ROM_NAME): $(BUILDDIR)/output_chr.bin $(BUILDDIR)/torchlight/torchlight_0.incs $(SOURCEDIR)/rainbow.cfg $(O_FILES)
	ld65 -vm -m $(BUILDDIR)/map.txt --dbgfile $(DBG_NAME) --define "__ZPCM_ADDRESS__=0x4011" --define "__BANK_MASK__=0x1F" --define "__BANK_OFFSET__=0x00" -o "build/tactus-zpcm.bin" -C $(SOURCEDIR)/rainbow.cfg $(O_FILES)
	ld65 -vm -m $(BUILDDIR)/map.txt                       --define "__ZPCM_ADDRESS__=0xFF11" --define "__BANK_MASK__=0x1F" --define "__BANK_OFFSET__=0x20" -o "build/tactus-base.bin" -C $(SOURCEDIR)/rainbow.cfg $(O_FILES)
